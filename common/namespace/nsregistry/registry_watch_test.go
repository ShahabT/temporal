...
func (s *registryWatchSuite) TestWatchEvents() {
	...
	s.NoError(err)
	s.Equal(ns1ID, ns.ID())
	s.Equal(cluster.TestCurrentClusterName, ns.ActiveClusterName(namespace.BusinessID{}))
	...
	s.NoError(err)
	s.Equal(ns1ID, ns.ID())
	s.Equal(cluster.TestAlternativeClusterName, ns.ActiveClusterName(namespace.BusinessID{}))
	...
	s.Equal(cluster.TestAlternativeClusterName, events[2].ns.ActiveClusterName(namespace.BusinessID{}))
...
func (s *registryWatchSuite) TestWatchUpdateWithoutStateChange() {
	...
	s.Equal(nsID, events[0].ns.ID())
	s.Equal(cluster.TestCurrentClusterName, events[0].ns.ActiveClusterName(namespace.BusinessID{}))
	s.False(events[0].deleted)

	// Second event: state change with new cluster (the retention-only update was skipped)
	s.Equal(nsID, events[1].ns.ID())
	s.Equal(cluster.TestAlternativeClusterName, events[1].ns.ActiveClusterName(namespace.BusinessID{}))
	s.False(events[1].deleted)
}
