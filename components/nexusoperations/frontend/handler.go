...
func (h *completionHandler) forwardCompleteOperation(ctx context.Context, r *nexusrpc.CompletionRequest, rCtx *requestContext) error {
	client, err := h.ForwardingClients.Get(rCtx.namespace.ActiveClusterName(namespace.BusinessID{ID: rCtx.workflowID}))
	if err != nil {
		h.Logger.Error("unable to get HTTP client for forward request", tag.Operation(apiName), tag.WorkflowNamespace(rCtx.namespace.Name().String()), tag.Error(err), tag.SourceCluster(h.ClusterMetadata.GetCurrentClusterName()), tag.TargetCluster(rCtx.namespace.ActiveClusterName(namespace.BusinessID{ID: rCtx.workflowID})))
	...
	if err != nil {
		h.Logger.Error("failed to construct forwarding request URL", tag.Operation(apiName), tag.WorkflowNamespace(rCtx.namespace.Name().String()), tag.Error(err), tag.TargetCluster(rCtx.namespace.ActiveClusterName(namespace.BusinessID{ID: rCtx.workflowID})))
		return nexus.NewHandlerErrorf(nexus.HandlerErrorTypeInternal, "internal error")
	}
	...
	if h.HTTPTraceProvider != nil {
		traceLogger := log.With(h.Logger,
			...
			tag.TargetCluster(rCtx.namespace.ActiveClusterName(namespace.BusinessID{ID: rCtx.workflowID})),
		)
...
func (c *requestContext) interceptRequest(ctx context.Context, request *nexusrpc.CompletionRequest) error {
	...
	if err := c.NamespaceValidationInterceptor.ValidateState(c.namespace, apiName, namespace.BusinessID{ID: c.workflowID}); err != nil {
		c.outcomeTag = metrics.OutcomeTag("invalid_namespace_state")
		return commonnexus.ConvertGRPCError(err, false)
	}

	// Redirect if current cluster is passive for this namespace.
	if c.namespace.ActiveClusterName(namespace.BusinessID{ID: c.workflowID}) != c.ClusterMetadata.GetCurrentClusterName() {
		if c.shouldForwardRequest(ctx, request.HTTPRequest.Header, c.workflowID) {
			...
			c.cleanupFunctions = append(c.cleanupFunctions, func(retErr error) {
				c.RedirectionInterceptor.AfterCall(handler, forwardStartTime, c.namespace.ActiveClusterName(namespace.BusinessID{ID: c.workflowID}), c.namespace.Name().String(), retErr)
			})
			...
			return serviceerror.NewNamespaceNotActive(c.namespace.Name().String(), c.ClusterMetadata.GetCurrentClusterName(), c.namespace.ActiveClusterName(namespace.BusinessID{ID: c.workflowID}))
		}
