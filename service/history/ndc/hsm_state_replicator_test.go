...
func (s *hsmStateReplicatorSuite) SetupTest() {
	...
	s.mockClusterMetadata.EXPECT().IsVersionFromSameCluster(cluster.TestCurrentClusterInitialFailoverVersion, s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})).Return(true).AnyTimes()
...
func (s *hsmStateReplicatorSuite) TestSyncHSM_WorkflowNotFound() {
	...
	err := s.nDCHSMStateReplicator.SyncHSMState(context.Background(), &historyi.SyncHSMRequest{
		...
		EventVersionHistory: &historyspb.VersionHistory{
			Items: []*historyspb.VersionHistoryItem{
				{EventId: lastEventID, Version: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})},
			},
		},
	})
	s.Error(err)
	...
	s.Equal(s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}), retryReplicationErr.EndEventVersion)
}
	...
	err := s.nDCHSMStateReplicator.SyncHSMState(context.Background(), &historyi.SyncHSMRequest{
		...
		EventVersionHistory: &historyspb.VersionHistory{
			Items: []*historyspb.VersionHistoryItem{
				...
				{EventId: 102, Version: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}) - 100},
			},
		},
	})
...
func (s *hsmStateReplicatorSuite) TestSyncHSM_Diverge_IncomingEventVersionLarger() {
	...
	err := s.nDCHSMStateReplicator.SyncHSMState(context.Background(), &historyi.SyncHSMRequest{
		...
		EventVersionHistory: &historyspb.VersionHistory{
			Items: []*historyspb.VersionHistoryItem{
				...
				{EventId: 80, Version: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}) - 100},
				{EventId: 202, Version: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}) + 100},
			},
		},
	})
	s.Error(err)
	...
	s.Equal(s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})-100, retryReplicationErr.StartEventVersion)
	s.Equal(int64(203), retryReplicationErr.EndEventId)
	s.Equal(s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})+100, retryReplicationErr.EndEventVersion)
}
	...
	err := s.nDCHSMStateReplicator.SyncHSMState(context.Background(), &historyi.SyncHSMRequest{
		...
		EventVersionHistory: &historyspb.VersionHistory{
			Items: []*historyspb.VersionHistoryItem{
				...
				{EventId: 50, Version: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}) - 100},
			},
		},
					...
					MachinesById: map[string]*persistencespb.StateMachineNode{
						"child1": {
							...
							InitialVersionedTransition: &persistencespb.VersionedTransition{
								NamespaceFailoverVersion: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}),
							},
							LastUpdateVersionedTransition: &persistencespb.VersionedTransition{
								NamespaceFailoverVersion: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}) + 100,
							},
							TransitionCount: 50,
						},
					},
...
func (s *hsmStateReplicatorSuite) TestSyncHSM_IncomingEventVersionSuperSet() {
	...
	err := s.nDCHSMStateReplicator.SyncHSMState(context.Background(), &historyi.SyncHSMRequest{
		...
		EventVersionHistory: &historyspb.VersionHistory{
			Items: []*historyspb.VersionHistoryItem{
				// incoming version large, should resend history
				{EventId: 50, Version: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}) - 100},
				{EventId: 202, Version: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})},
				{EventId: 302, Version: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}) + 100},
			},
		},
	})
	s.Error(err)
	...
	s.Equal(s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}), retryReplicationErr.StartEventVersion)
					...
					MachinesById: map[string]*persistencespb.StateMachineNode{
						"child1": {
							...
							LastUpdateVersionedTransition: &persistencespb.VersionedTransition{
								...
								NamespaceFailoverVersion: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}) + 50,
							},
							TransitionCount: 50,
						},
					},
					...
					MachinesById: map[string]*persistencespb.StateMachineNode{
						"child1": {
							...
							LastUpdateVersionedTransition: &persistencespb.VersionedTransition{
								...
								NamespaceFailoverVersion: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}) + 200,
							},
							TransitionCount: 50,
						},
					},
...
func (s *hsmStateReplicatorSuite) TestSyncHSM_StateMachineNotFound() {
	...
	baseVersion := s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})
	persistedState := s.buildWorkflowMutableState()
			...
			Histories: []*historyspb.VersionHistory{
				{
					...
					Items: []*historyspb.VersionHistoryItem{
						{EventId: 50, Version: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}) - 100},
						{EventId: 102, Version: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})},
					},
				},
			},
		},
