...
func GetActiveNamespace(
	...
	if namespaceEntry.ActiveClusterName(namespace.BusinessID{ID: businessID}) != shard.GetClusterMetadata().GetCurrentClusterName() {
		return nil, serviceerror.NewNamespaceNotActive(
			namespaceEntry.Name().String(),
			shard.GetClusterMetadata().GetCurrentClusterName(),
			namespaceEntry.ActiveClusterName(namespace.BusinessID{ID: businessID}))
	}
