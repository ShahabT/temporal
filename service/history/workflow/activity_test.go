...
func (s *activitySuite) SetupTest() {
	...
	s.mockShard.Resource.ClusterMetadata.EXPECT().ClusterNameForFailoverVersion(true, tests.GlobalNamespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})).Return(cluster.TestCurrentClusterName).AnyTimes()
