...
func (s *mutableStateSuite) SetupTest() {
	...
	s.mockShard.Resource.ClusterMetadata.EXPECT().ClusterNameForFailoverVersion(s.namespaceEntry.IsGlobalNamespace(), s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})).Return(cluster.TestCurrentClusterName).AnyTimes()
...
func (s *mutableStateSuite) buildWorkflowMutableState() *persistencespb.WorkflowMutableState {
	...
	tl := "testTaskQueue"
	failoverVersion := s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})
...
func (s *mutableStateSuite) TestUpdateInfos() {
	...
	s.NoError(err)
	err = s.mutableState.UpdateCurrentVersion(namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}), false)
	s.NoError(err)
	...
	s.mockShard.Resource.ClusterMetadata.EXPECT().ClusterNameForFailoverVersion(
		...
		namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}),
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			...
			if tc.versionedTransitionUpdated {
				expectedTransitionHistory = UpdatedTransitionHistory(expectedTransitionHistory, namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}))
			}
...
func (s *mutableStateSuite) TestGetCloseVersion() {
	...
	s.NoError(err)
	expectedVersion := namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})
...
func (s *mutableStateSuite) TestVersionedTransitionInDB() {
	...
	ms.executionInfo.TransitionHistory = UpdatedTransitionHistory(ms.executionInfo.TransitionHistory, s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}))
	s.NoError(ms.cleanupTransaction())
	s.True(proto.Equal(ms.CurrentVersionedTransition(), ms.versionedTransitionInDB))
}
...
func (s *mutableStateSuite) TestCloseTransactionTrackTombstones_CapIfLargerThanLimit() {
	...
	for i := 0; i < s.mockConfig.MutableStateTombstoneCountLimit(); i++ {
		signalMap[int64(76+i)] = &persistencespb.SignalInfo{
			...
			Version:               s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}),
...
func (s *mutableStateSuite) TestCloseTransactionGenerateCHASMRetentionTask_NonWorkflow_Passive() {
	...
	updatedExecutionState.LastUpdateVersionedTransition = &persistencespb.VersionedTransition{
		NamespaceFailoverVersion: s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: s.mutableState.executionInfo.WorkflowId}),
