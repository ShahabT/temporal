...
func (s *taskRefresherSuite) SetupTest() {
	...
	s.mockShard.Resource.ClusterMetadata.EXPECT().ClusterNameForFailoverVersion(true, s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID})).Return(cluster.TestCurrentClusterName).AnyTimes()
	s.mutableState = TestGlobalMutableState(
		s.mockShard,
		s.mockShard.GetEventsCache(),
		s.mockShard.GetLogger(),
		s.namespaceEntry.FailoverVersion(namespace.BusinessID{ID: tests.WorkflowID}),
		tests.WorkflowID,
		tests.RunID,
	)
