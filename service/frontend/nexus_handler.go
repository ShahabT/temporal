...
func (c *operationContext) interceptRequest(
	...
	if err := c.namespaceValidationInterceptor.ValidateState(c.namespace, c.apiName, namespace.BusinessID{}); err != nil {
	...
	if !c.namespace.ActiveInCluster(c.clusterMetadata.GetCurrentClusterName()) {
		if c.shouldForwardRequest(ctx, header) {
			...
			c.cleanupFunctions = append(c.cleanupFunctions, func(_ map[string]string, retErr error) {
				c.redirectionInterceptor.AfterCall(handler, forwardStartTime, c.namespace.ActiveClusterName(namespace.BusinessID{}), c.namespace.Name().String(), retErr)
			})
			return serviceerror.NewNamespaceNotActive(
				c.namespaceName,
				c.clusterMetadata.GetCurrentClusterName(),
				c.namespace.ActiveClusterName(namespace.BusinessID{}),
			)
		}
...
func (h *nexusHandler) forwardStartOperation(
	...
	if h.httpTraceProvider != nil {
		traceLogger := log.With(h.logger,
			...
			tag.TargetCluster(oc.namespace.ActiveClusterName(namespace.BusinessID{})),
		)
...
func (h *nexusHandler) nexusClientForActiveCluster(oc *operationContext, service string) (*nexusrpc.HTTPClient, error) {
	httpClient, err := h.forwardingClients.Get(oc.namespace.ActiveClusterName(namespace.BusinessID{}))
	if err != nil {
		oc.logger.Error("failed to forward Nexus request. error creating HTTP client", tag.Error(err), tag.SourceCluster(oc.namespace.ActiveClusterName(namespace.BusinessID{})), tag.TargetCluster(oc.namespace.ActiveClusterName(namespace.BusinessID{})))
